...
REGISTRY: list[DomainSpec] = [
    DomainSpec(
        "country_of_birth", "C21_T08_SA2", "bplp",
        "Country of birth of person (top countries), % of persons.",
        timeseries=True, exclude_labels=("Total",), denominator_label="Total",
    ),
    DomainSpec(
        "language", "C21_T10_SA2", "lanp",
        ...
        exclude_labels=("Total", "Chinese: Total"), denominator_label="Total",
    ),
    DomainSpec(
        "education_institution", "C21_T13_SA2", "tystap",
        "Type of educational institution attending, % of persons attending.",
        timeseries=True, denominator_label="Total",
        include_labels=(
            "Preschool", "Primary - Total", "Secondary - Total",
            "University or other higher education - Total",
            "Vocational education (including TAFE and private training providers) - Total",
            "Other - Total", "Type of educational institution not stated",
        ),
    ),
    DomainSpec(
        "labour_force", "C21_T33_SA2", "lfsp",
        ...
        include_labels=(
            ...
            "Not in the labour force", "Labour force status not stated",
        ),
    ),
    DomainSpec(
        "income_personal", "C21_G17_SA2", "incp",
        "Total personal income (weekly) brackets, % of persons 15+.",
        exclude_labels=("Total",), denominator_label="Total",
    ),
    DomainSpec(
        "family_composition", "C21_T14_SA2", "hhcfmcd",
        "Family composition (within family households), % of family households.",
        timeseries=True, denominator_label="Family Household: Total",
        include_labels=(
    ...
    DomainSpec(
        "household_composition", "C21_T14_SA2", "hhcfmcd",
        "Household composition, % of households.",
        timeseries=True, denominator_label="Total",
        include_labels=(
            "Family Household: Total", "Group household",
            "Lone person household", "Other households",
        ),
    ),
    DomainSpec(
        "tenure", "C21_T18_SA2", "tenlld",
        "Tenure and landlord type, % of occupied dwellings.",
        timeseries=True, denominator_label="Total",
        include_labels=(
            "Owned outright", "Owned with a mortgage", "Rented: Total",
            "Other tenure type", "Tenure type not stated",
        ),
    ),
]
