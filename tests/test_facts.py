...
def _frame():
    ...
    for sex, mult in [("Persons", 1.0), ("Males", 0.5)]:
        for cat, val in [("A", 30), ("B", 70), ("Total", 100)]:
            rows.append({"sa2_code": "999", "year": 2021, "sexp_label": sex,
                         "cat_label": cat, "value": val * mult})
...
def test_marginalises_to_persons_and_excludes_total():
    ...
    assert rec["A"]["count"] == 30 and rec["A"]["denominator"] == 100 and rec["A"]["proportion"] == 30.0
    assert rec["B"]["proportion"] == 70.0


def test_include_labels_and_zero_denominator_null():
    df = pl.DataFrame({
        "sa2_code": ["0", "0", "0"], "year": [2021, 2021, 2021],
        "cat_label": ["Leaf1", "Leaf2", "Total"], "value": [0.0, 0.0, 0.0],
    })
    spec = DomainSpec("x", "F", "cat", "d",
                      include_labels=("Leaf1", "Leaf2"), denominator_label="Total")
